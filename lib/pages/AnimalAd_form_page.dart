...
class AnimalAdFormPage extends StatefulWidget {
  const AnimalAdFormPage({super.key});

  @override
  State<AnimalAdFormPage> createState() => _AnimalAdFormPageState();
}

class _AnimalAdFormPageState extends State<AnimalAdFormPage> {
 ...
 String? emailUser = Auth.emailUserForm;
  ...
  Future<UploadTask> upload(String path) async{
    ...
    File file = File(path);
    try{
      String ref = 'imagesAnimalAd/img-${DateTime.now().toString()}.jpg';
  ...
  void didChangeDependencies() {
    ...
    if(_formData.isEmpty){
      ...
      if(arg != null){
        final  animalAd = arg as AnimalAdModel;
        if(animalAd.imagem != null)
        urlImage = animalAd.imagem;
        _formData['idAnimalAd'] = animalAd.idAnimalAd;
        _formData['emailUser'] = animalAd.emailUser == null || animalAd.emailUser == ''  ? emailUser.toString() : animalAd.emailUser;
        _formData['nome'] = animalAd.nome;
        _formData['nomeResp'] = animalAd.nomeResp;
        _formData['contatoResp'] = animalAd.contatoResp;
        _formData['enderecoResp'] = animalAd.enderecoResp;
        _formData['descricao'] = animalAd.descricao;     
         debugPrint('Entrou ${animalAd.imagem}');      
        _formData['imagem'] = urlImage ;
        //_formData['imagem'] = animalAd.imagem;

        //_imageUrlController.text  = animalAd.imagem;
      }
    }
  }
  ...
  Future<void> _submitForm() async{
    ...
    try{
      await Provider.of<AnimalAdList>(
      context, 
      listen: false,
      ).saveAnimalAd(_formData);
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              //   ],
              // ),
              //AnimalAdImagePicker(onImagePick: _handleImagePicker,),
              TextFormField(
                ...
                validator: (_nome){
                  ...
                  if(nome.trim().isEmpty){
                    return 'O nome do pet não pode ser vazio!';
                  }
